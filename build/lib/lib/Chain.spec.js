...
import { ORG_NAME, ORG_DOMAIN, ORG_KEY_PAIR, ORG_KEY_SPEC, } from '../testUtils/veraStubs/organisation.js';
...
const { orgCertificate, memberCertificate, dnssecChainFixture, datePeriod } = await generateMemberIdFixture();
...
class StubChain extends Chain {
    constructor(veraidDnssecChain, organisationCertificate, signerCertificateValue, signerNameValue) {
        super(veraidDnssecChain, organisationCertificate);
        this.signerCertificateValue = signerCertificateValue;
        this.signerNameValue = signerNameValue;
    }
    ...
    describe('verify', () => {
        describe('Certificate chain', () => {
            test('Signer certificate should be issued by organisation certificate', async () => {
                const otherOrgCertificate = await selfIssueOrganisationCertificate(ORG_NAME, await generateRsaKeyPair(), datePeriod.end, { startDate: datePeriod.start });
                const otherDnssecChain = new VeraidDnssecChain(otherOrgCertificate.commonName, dnssecChainFixture.responses.map(serialiseMessage).map(bufferToArray));
                const chain = new StubChain(otherDnssecChain, otherOrgCertificate, memberCertificate);
                const error = await getPromiseRejection(async () => chain.verify(SERVICE_OID, datePeriod), VeraidError);
                expectErrorToEqual(error, new VeraidError('Member certificate was not issued by organisation', {
            ...
            test('Certificates should overlap with specified period', async () => {
                ...
                await expect(async () => chain.verify(SERVICE_OID, pastPeriod)).rejects.toThrowWithMessage(VeraidError, `Validity period of certificate chain (${datePeriod.toString()}) ` +
                    `does not overlap with required period (${pastPeriod.toString()})`);
            });
        });
        describe('User name validation', () => {
            const errorMessage = 'User name should not contain at signs or whitespace other than simple spaces';
            test('should not contain at signs', async () => {
                const invalidSignerName = `@${MEMBER_NAME}`;
                const chain = new StubChain(VERAID_DNSSEC_CHAIN, orgCertificate, memberCertificate, invalidSignerName);
                await expect(async () => chain.verify(SERVICE_OID, datePeriod, dnssecChainFixture.trustAnchors)).rejects.toThrowWithMessage(VeraidError, errorMessage);
            });
        ...
        describe('DNSSEC chain', () => {
            test('Service OID should be verified', async () => {
                ...
                expect(chainVerificationSpy).toHaveBeenCalledWith(expect.anything(), SERVICE_OID, expect.anything(), expect.anything());
            });
            test('Key spec should match that set in TXT rdata', async () => {
                const chain = new StubChain(VERAID_DNSSEC_CHAIN, orgCertificate, memberCertificate);
                const chainVerificationSpy = jest.spyOn(VeraidDnssecChain.prototype, 'verify');
                await chain.verify(SERVICE_OID, datePeriod, dnssecChainFixture.trustAnchors);
                expect(chainVerificationSpy).toHaveBeenCalledWith(ORG_KEY_SPEC, expect.anything(), expect.anything(), expect.anything());
            });
            test('Date period should be intersection of specified one and the certificates', async () => {
                const chain = new StubChain(VERAID_DNSSEC_CHAIN, orgCertificate, memberCertificate);
                const chainVerificationSpy = jest.spyOn(VeraidDnssecChain.prototype, 'verify');
                const narrowPeriod = DatePeriod.init(subSeconds(datePeriod.start, 1), subSeconds(datePeriod.end, 1));
                ...
                expect(chainVerificationSpy).toHaveBeenCalledWith(expect.anything(), expect.anything(), intersection, expect.anything());
            });
            test('Verification errors should be propagated', async () => {
                const chain = new StubChain(VERAID_DNSSEC_CHAIN, orgCertificate, memberCertificate);
                // Do not pass trusted anchors
                await expect(async () => chain.verify(SERVICE_OID, datePeriod)).rejects.toThrowWithMessage(VeraidError, /^VeraId DNSSEC chain is BOGUS/u);
            });
        });
        describe('Valid result', () => {
            test('Organisation name should be output', async () => {
                const chain = new StubChain(VERAID_DNSSEC_CHAIN, orgCertificate, memberCertificate);
                const { organisation } = await chain.verify(SERVICE_OID, datePeriod, dnssecChainFixture.trustAnchors);
                expect(organisation).toStrictEqual(ORG_NAME);
            });
            test('Trailing dot should be removed from domain if present', async () => {
                const otherOrgCertificate = await selfIssueOrganisationCertificate(ORG_DOMAIN, ORG_KEY_PAIR, datePeriod.end, { startDate: datePeriod.start });
                ...
                const chain = new StubChain(VERAID_DNSSEC_CHAIN, otherOrgCertificate, fixture.memberCertificate);
                const { organisation } = await chain.verify(SERVICE_OID, datePeriod, fixture.dnssecChainFixture.trustAnchors);
                expect(organisation).toStrictEqual(ORG_NAME);
                expect(organisation).not.toEndWith('.');
            });
            test('User name should be output if provided', async () => {
                const chain = new StubChain(VERAID_DNSSEC_CHAIN, orgCertificate, memberCertificate, MEMBER_NAME);
                const { user } = await chain.verify(SERVICE_OID, datePeriod, dnssecChainFixture.trustAnchors);
