...
import { Attribute, EncapsulatedContentInfo, IssuerAndSerialNumber, RelativeDistinguishedNames, SignedAndUnsignedAttributes, SignedData as PkijsSignedData, SignerInfo, } from 'pkijs';
...
describe('sign', () => {
    ...
    describe('SignerInfo', () => {
        ...
        describe('SignedAttributes', () => {
            ...
            test('Content type attribute should be set to CMS Data', async () => {
                ...
                expect(contentTypeAttribute.values[0].valueBlock.toString()).toStrictEqual(CMS_OIDS.DATA);
            });
            test('Plaintext digest should be present', async () => {
                ...
                const digest = digestAttribute.values[0].valueBlock.valueHexView;
    ...
    describe('Extra signed attributes', () => {
        ...
        test('Any extra attributes should be honored', async () => {
            ...
            const signedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, certificate, [], { extraSignedAttrs: [attribute] });
            const attachedAttribute = getSignerInfoAttribute(signedData.pkijsSignedData.signerInfos[0], attribute.type);
            expect(Buffer.from(attachedAttribute.values[0].valueBlock.valueHexView)).toStrictEqual(Buffer.from(attribute.values[0].valueBlock.valueHexView));
        });
    });
    describe('Hashing', () => {
        test('SHA-256 should be used by default', async () => {
            const signedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, certificate);
            const digestAttribute = getSignerInfoAttribute(signedData.pkijsSignedData.signerInfos[0], CMS_OIDS.ATTR_DIGEST);
            expect(Buffer.from(digestAttribute.values[0].valueBlock.valueHexView)).toStrictEqual(calculateDigest('sha256', plaintext));
        });
        ...
        test('SHA-1 should not be a valid hashing function', async () => {
            await expect(async () => SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, certificate, [], {
    ...
    describe('Plaintext', () => {
        test('Plaintext should be encapsulated by default', async () => {
            ...
            expect(Buffer.from(plaintext)).toStrictEqual(Buffer.from(plaintextOctetString.valueBlock.valueHexView.slice().buffer));
        });
        test('Content should not be encapsulated if requested', async () => {
            const signedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, certificate, undefined, { shouldEncapsulatePlaintext: false });
            const { encapContentInfo } = signedData.pkijsSignedData;
            expect(encapContentInfo).toBeInstanceOf(EncapsulatedContentInfo);
            expect(encapContentInfo.eContentType).toBe(CMS_OIDS.DATA);
...
describe('serialize', () => {
    test('SignedData value should be wrapped in ContentInfo', async () => {
        ...
        expect(asn1Serialise(contentInfo.content)).toStrictEqual(pkijsSerialise(signedData.pkijsSignedData));
    });
...
describe('deserialize', () => {
    test('A non-DER-encoded value should be refused', () => {
        ...
        expectFunctionToThrowError(() => SignedData.deserialize(invalidSignature), new CmsError('Could not deserialize CMS ContentInfo', {
    ...
    test('Malformed SignedData values should be refused', () => {
        ...
        expectFunctionToThrowError(() => SignedData.deserialize(invalidSignature), new CmsError('SignedData value is malformed', { cause: expect.anything() }));
    });
...
describe('verify', () => {
    test('Value should be refused if plaintext is not encapsulated or specified', async () => {
        const signedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, certificate, undefined, {
    ...
    test('Expected plaintext should be refused if one is already encapsulated', async () => {
        const signedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, certificate);
        await expect(signedData.verify(plaintext)).rejects.toThrowWithMessage(CmsError, 'No specific plaintext should be expected because one is already encapsulated');
    });
    ...
    test('Invalid signature should be rejected', async () => {
        ...
        await expect(signedData.verify()).rejects.toThrowWithMessage(CmsError, 'Invalid signature (PKI.js code: 14)');
    });
    test('Valid signature without encapsulated plaintext should be accepted', async () => {
        const signedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, certificate, [certificate], {
            shouldEncapsulatePlaintext: false,
        });
    ...
    describe('Detached signer certificate', () => {
        ...
        test('Should not alter original signed data', async () => {
            ...
            expect(Buffer.from(signedData.serialize())).toStrictEqual(Buffer.from(originalSignedDataSerialised));
        });
        test('Should reject when provided signer certificate does not match', async () => {
            ...
            await expect(signedData.verify(undefined, differentCertificate)).rejects.toThrowWithMessage(CmsError, 'Invalid signature');
        });
        ...
        test('Should verify when signer matches and certificates is undefined', async () => {
            const originalSignedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, certificate);
...
describe('plaintext', () => {
    ...
    test('Large plaintexts chunked by PKI.js should be put back together', async () => {
        ...
        const signedData = await SignedData.sign(largePlaintext, MEMBER_KEY_PAIR.privateKey, certificate);
...
describe('signerCertificate', () => {
    test('Nothing should be output if there are no SignerInfo values', async () => {
        ...
        const signedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, signerCertificate, [signerCertificate]);
    ...
    test('Certificate with same issuer but different SN should be ignored', async () => {
        ...
        signedData.pkijsSignedData.signerInfos.forEach((signerInfo) => {
            ...
            signerInfo.sid.serialNumber = new Integer({
                value: -1,
            });
        });
        expect(signedData.signerCertificate).toBeNull();
    ...
    test('Certificate with same SN but different issuer should be ignored', async () => {
        ...
        signedData.pkijsSignedData.signerInfos.forEach((info) => {
            // eslint-disable-next-line no-param-reassign
            info.sid.issuer = new RelativeDistinguishedNames();
        });
        expect(signedData.signerCertificate).toBeNull();
    });
...
describe('signerIssuerAndSerialNumber', () => {
    test('Nothing should be output if there are no SignerInfo values', async () => {
        ...
        const signedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, signerCertificate);
        signedData.pkijsSignedData.signerInfos.pop();
...
describe('certificates', () => {
    test('Attached certificates should be output', async () => {
        ...
        const signedData = await SignedData.sign(plaintext, signerKeyPair.privateKey, signerCertificate, [signerCertificate, intermediateCaCertificate, rootCaCertificate]);
...
describe('getSignedAttribute', () => {
    test('Error should be thrown if there are no signers', async () => {
        const signedData = await SignedData.sign(plaintext, MEMBER_KEY_PAIR.privateKey, certificate);
        signedData.pkijsSignedData.signerInfos.pop();
        expect(() => signedData.getSignedAttribute(CMS_OIDS.ATTR_DIGEST)).toThrowWithMessage(CmsError, 'SignedData value does not have any signers');
    });
    ...
    test('Attribute content should be returned if attribute is presented', async () => {
        ...
        expect(contentTypeValues[0].valueBlock.toString()).toStrictEqual(CMS_OIDS.DATA);
    });
});
