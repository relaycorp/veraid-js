...
import { AuthorityKeyIdentifier, Certificate as PkijsCertificate, CryptoEngine, IssuerAndSerialNumber, PublicKeyInfo, } from 'pkijs';
...
describe('constructor', () => {
    describe('validityPeriod', () => {
        test('Start date after expiry date should be refused', async () => {
            ...
            expect(() => new Certificate(pkijsCertificate)).toThrowWithMessage(Error, /^End date should not be before start date/u);
        });
...
describe('deserialize()', () => {
    test('should deserialize valid DER-encoded certificates', async () => {
        ...
        expect(cert.pkijsCertificate.subject.typesAndValues[0].type).toBe(pkijsCertificate.subject.typesAndValues[0].type);
        expect(cert.pkijsCertificate.subject.typesAndValues[0].value.valueBlock.value).toBe(pkijsCertificate.subject.typesAndValues[0].value.valueBlock.value);
    });
...
describe('issue()', () => {
    ...
    test('should be signed with the specified private key', async () => {
        ...
        expect(signSpy).toHaveBeenCalledWith(subjectKeyPair.privateKey, subjectKeyPair.privateKey.algorithm.hash.name, NODE_ENGINE);
    });
    ...
    test('should generate a positive serial number', async () => {
        const certificates = await Promise.all(Array.from({ length: 10 }, async () => Certificate.issue({
            ...baseCertificateOptions,
            issuerPrivateKey: subjectKeyPair.privateKey,
            subjectPublicKey: subjectKeyPair.publicKey,
        })));
    ...
    test('should refuse an issuer certificate without extensions', async () => {
        ...
        await expect(Certificate.issue({
            ...
            issuerPrivateKey: issuerKeyPair.privateKey,
            subjectPublicKey: subjectKeyPair.publicKey,
        })).rejects.toStrictEqual(new CertificateError('Basic constraints extension is missing from issuer certificate'));
    });
    ...
    test('should refuse an issuer certificate without basic constraints extension', async () => {
        ...
        invalidIssuerCertificate.pkijsCertificate.extensions =
            invalidIssuerCertificate.pkijsCertificate.extensions.filter((extension) => extension.extnID !== BASIC_CONSTRAINTS);
        await expect(Certificate.issue({
            ...baseCertificateOptions,
            issuerCertificate: invalidIssuerCertificate,
            issuerPrivateKey: subjectKeyPair.privateKey,
    ...
    describe('Authority Key Identifier extension', () => {
        function getAkiExtension(subjectCert) {
        ...
        test('should correspond to subject when self-issued', async () => {
            ...
            expect(keyIdBuffer.toString('hex')).toStrictEqual(await getPublicKeyDigest(subjectKeyPair.publicKey));
        });
        test('should correspond to issuer key when different from subject', async () => {
            ...
            expect(akiExtension.keyIdentifier).toBeInstanceOf(OctetString);
            const keyIdBuffer = Buffer.from(akiExtension.keyIdentifier.valueBlock.valueHexView);
            expect(keyIdBuffer.toString('hex')).toStrictEqual(await getPublicKeyDigest(issuerKeyPair.publicKey));
        });
    });
    test('Subject Key Identifier extension should correspond to subject key', async () => {
        ...
        const keyIdBuffer = Buffer.from(skiExtensionAsn1.valueBlock.valueHexView);
        expect(keyIdBuffer.toString('hex')).toStrictEqual(await getPublicKeyDigest(subjectKeyPair.publicKey));
    });
});
...
test('getSerialNumber() should return the serial number as a buffer', async () => {
    const cert = await generateStubCert();
    expect(cert.serialNumber).toStrictEqual(Buffer.from(cert.pkijsCertificate.serialNumber.valueBlock.valueHexView));
});
describe('commonName', () => {
    ...
    test('should error out when the address is not found', async () => {
        ...
        expect(() => cert.commonName).toThrowWithMessage(CertificateError, 'Distinguished Name does not contain Common Name');
    });
});
describe('isEqual', () => {
...
describe('getCertificationPath', () => {
    ...
    test('Cert not issued by trusted cert should not be trusted', async () => {
        const cert = await generateStubCert();
        await expect(cert.getCertificationPath([], [stubRootCa])).rejects.toStrictEqual(new CertificateError('No valid certificate paths found'));
    });
    ...
    test('Cert issued by untrusted intermediate should be trusted if root is trusted', async () => {
        ...
        await expect(cert.getCertificationPath([intermediateCaCert], [stubRootCa])).resolves.toStrictEqual([cert, intermediateCaCert, stubRootCa]);
    });
    ...
    test('Cert issued by untrusted intermediate CA should not be trusted', async () => {
        ...
        await expect(cert.getCertificationPath([untrustedIntermediateCaCert], [stubRootCa])).rejects.toStrictEqual(new CertificateError('No valid certificate paths found'));
    });
    test('Including trusted intermediate CA should not make certificate trusted', async () => {
        ...
        const cert = await generateStubCert();
        await expect(cert.getCertificationPath([trustedIntermediateCaCert], [stubRootCa])).rejects.toStrictEqual(new CertificateError('No valid certificate paths found'));
    });
    test('Root certificate should be ignored if passed as intermediate unnecessarily', async () => {
        ...
        await expect(cert.getCertificationPath([intermediateCaCert, stubRootCa], [intermediateCaCert])).resolves.toStrictEqual([cert, intermediateCaCert]);
    });
});
test('getPublicKey should return the subject public key', async () => {
    ...
    await expect(derSerializePublicKey(publicKey)).resolves.toStrictEqual(await derSerializePublicKey(subjectKeyPair.publicKey));
});
