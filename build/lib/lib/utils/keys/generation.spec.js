...
import { generateRsaKeyPair, getRsaPublicKeyFromPrivate, } from './generation.js';
...
describe('generateRsaKeyPair', () => {
    ...
    describe('Modulus', () => {
        ...
        test('Modulus < 2048 should not supported', async () => {
            await expect(generateRsaKeyPair({ modulus: 1024 })).rejects.toThrow('RSA modulus must be => 2048 (got 1024)');
        });
    });
    describe('Hashing algorithm', () => {
        ...
        test('SHA-1 should not be supported', async () => {
            await expect(generateRsaKeyPair({ hashingAlgorithm: 'SHA-1' })).rejects.toThrow('SHA-1 is unsupported');
        });
    });
});
describe('getRsaPublicKeyFromPrivate', () => {
    test('Public key should be returned', async () => {
        ...
        await expect(derSerializePublicKey(publicKey)).resolves.toStrictEqual(await derSerializePublicKey(keyPair.publicKey));
    });
    test('Public key should be taken from provider if custom one is used', async () => {
        const keyPair = await generateRsaKeyPair();
        const mockRsaPssProvider = new MockRsaPssProvider();
        mockRsaPssProvider.onExportKey.mockResolvedValue(await derSerializePublicKey(keyPair.publicKey));
...
//# sourceMappingURL=generation.spec.js.map