...
import { derDeserializeRsaPrivateKey, derDeserializeRsaPublicKey, derSerializePrivateKey, derSerializePublicKey, } from './serialisation.js';
...
describe('Key serializers', () => {
    ...
    describe('derSerializePublicKey', () => {
        ...
        test('Public key should be extracted first if input is PrivateKey', async () => {
            ...
            await expect(derSerializePublicKey(privateKey)).resolves.toStrictEqual(Buffer.from(stubExportedKeyDer));
...
describe('Key deserializers', () => {
    ...
    test('derDeserializeRsaPublicKey should convert DER public key to RSA key', async () => {
        ...
        expect(mockImportKey).toHaveBeenCalledWith('spki', bufferToArray(stubKeyDer), rsaAlgorithmOptions, true, ['verify']);
    });
    ...
    test('derDeserializeRsaPublicKey should accept an ArrayBuffer serialization', async () => {
        ...
        expect(publicKey).toBe(STUB_KEY_PAIR.publicKey);
        expect(mockImportKey).toHaveBeenCalledTimes(1);
        expect(mockImportKey).toHaveBeenCalledWith('spki', keyDerArrayBuffer, rsaAlgorithmOptions, true, ['verify']);
    });
    test('derDeserializeRSAPrivateKey should convert DER private key to RSA key', async () => {
        ...
        expect(mockImportKey).toHaveBeenCalledTimes(1);
        expect(mockImportKey).toHaveBeenCalledWith('pkcs8', bufferToArray(stubKeyDer), rsaAlgorithmOptions, true, ['sign']);
    });
