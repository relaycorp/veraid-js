...
describe('getPublicKeyDigest', () => {
    test('SHA-256 digest should be returned in hex', async () => {
        ...
        expect(Buffer.from(digest)).toStrictEqual(createHash('sha256')
    ...
    test('Public key should be extracted first if input is private key', async () => {
        ...
        expect(Buffer.from(digest)).toStrictEqual(createHash('sha256').update(Buffer.from(mockPublicKeySerialized)).digest());
    });
});
//# sourceMappingURL=digest.spec.js.map