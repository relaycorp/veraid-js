...
describe('MemberIdBundle', () => {
    ...
    describe('fromSchema', () => {
        test('should create instance from valid schema', () => {
            ...
            expect(Buffer.from(bundleFromSchema.serialise())).toStrictEqual(Buffer.from(bundle.serialise()));
        });
    });
    describe('serialise', () => {
        ...
        test('DNSSEC chain should be included', () => {
            ...
            expect(Buffer.from(dnssecChainSerialised)).toStrictEqual(Buffer.from(VERAID_DNSSEC_CHAIN.serialise()));
        });
    ...
    describe('signerName', () => {
        ...
        test('should return undefined if member is a bot', async () => {
            const botCertificate = await issueMemberCertificate(undefined, MEMBER_KEY_PAIR.publicKey, orgCertificate, ORG_KEY_PAIR.privateKey, datePeriod.end, { startDate: datePeriod.start });
    ...
    describe('deserialise', () => {
        test('Malformed member Id bundle should be refused', () => {
            ...
            expect(() => MemberIdBundle.deserialise(malformedBundle)).toThrowWithMessage(VeraidError, 'Member id bundle is malformed');
        });
        test('should deserialise valid bundle', () => {
            const dnssecChain = new VeraidDnssecChain(orgCertificate.commonName, new DnssecChainSchema(dnssecChainFixture.responses.map(serialiseMessage).map(bufferToArray)));
            ...
            expect(Buffer.from(deserializedBundle.serialise())).toStrictEqual(Buffer.from(bundleSerialised));
        });
    });
});
//# sourceMappingURL=MemberIdBundle.spec.js.map