...
describe('selfIssueOrganisationCertificate', () => {
    ...
    test('Public key should match specified one', async () => {
        const certificate = await selfIssueOrganisationCertificate(ORG_NAME, ORG_KEY_PAIR, EXPIRY_DATE);
        await expect(derSerializePublicKey(await certificate.getPublicKey())).resolves.toStrictEqual(await derSerializePublicKey(ORG_KEY_PAIR.publicKey));
    });
    test('Certificate should be signed with private key', async () => {
        ...
        expect(certificateIssueSpy).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('Start date', () => {
        ...
        test('should match explicit date if set', async () => {
            const certificate = await selfIssueOrganisationCertificate(ORG_NAME, ORG_KEY_PAIR, EXPIRY_DATE, { startDate: START_DATE });
