...
describe('init', () => {
    ...
    test('Start date after end date should be refused', () => {
        ...
        expect(() => DatePeriod.init(start, end)).toThrowWithMessage(Error, `End date should not be before start date (${start.toISOString()} <= ${end.toISOString()})`);
    });
});
describe('overlaps', () => {
...
describe('intersect', () => {
    ...
    test('Undefined should be returned if periods do not overlap', () => {
        const otherPeriod = DatePeriod.init(addSeconds(stubPeriod.end, 1), addSeconds(stubPeriod.end, 2));
