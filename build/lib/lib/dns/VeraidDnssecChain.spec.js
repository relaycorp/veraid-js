...
import { Message, Question, RrSet, SecurityStatus } from '@relaycorp/dnssec';
...
import { ORG_DOMAIN, ORG_KEY_PAIR, ORG_KEY_SPEC, ORG_VERAID_DOMAIN, VERAID_RECORD, VERAID_RECORD_TTL_OVERRIDE, } from '../../testUtils/veraStubs/organisation.js';
...
const mockDnssecOnlineResolve = jest.fn();
...
describe('VeraidDnssecChain', () => {
    describe('retrieve', () => {
        function makeRetrievalOptions(status) {
        ...
        test('IANA trust anchors should be used by default', async () => {
            ...
            await expect(VeraidDnssecChain.retrieve(ORG_DOMAIN)).rejects.toThrowWithMessage(VeraidError, 'DNSSEC chain validation failed (BOGUS): ' +
        ...
        test('Online DNSSEC resolver should be used by default', async () => {
            const { resolver, trustAnchors } = MOCK_CHAIN.generateFixture(VERAID_RRSET, SecurityStatus.SECURE);
            mockDnssecOnlineResolve.mockImplementation(resolver);
        ...
        test('TXT subdomain _veraid of specified domain should be queried', async () => {
            ...
            const resolverSpy = jest.fn(resolver);
            await VeraidDnssecChain.retrieve(ORG_DOMAIN, { resolver: resolverSpy, trustAnchors });
            expect(resolverSpy).toHaveBeenCalledWith(expect.toSatisfy((question) => question.name === ORG_VERAID_DOMAIN && question.getTypeName() === 'TXT'));
        });
        test('Errors should be wrapped', async () => {
            const resolver = jest.fn();
            const originalError = new Error('Whoops');
            resolver.mockRejectedValue(originalError);
            const error = await getPromiseRejection(async () => VeraidDnssecChain.retrieve(ORG_DOMAIN, { resolver }), VeraidError);
        ...
        test('Non-SECURE result should be refused', async () => {
            ...
            const error = await getPromiseRejection(async () => VeraidDnssecChain.retrieve(ORG_DOMAIN, options), VeraidError);
        ...
        test('Responses in wire format should be supported', async () => {
            const { resolver, trustAnchors } = makeRetrievalOptions(SecurityStatus.SECURE);
            const mockResolver = async (question) => {
                const response = (await resolver(question));
                return serialiseMessage(response);
            };
            await expect(VeraidDnssecChain.retrieve(ORG_DOMAIN, { resolver: mockResolver, trustAnchors })).toResolve();
        });
        ...
        test('Responses should be stored in instance', async () => {
            ...
            const responses = await Promise.all(resolverSpy.mock.results.map((promise) => promise.value));
    ...
    describe('fromSchema', () => {
        test('should create instance from valid schema', () => {
            ...
            const mapToBuffer = (arr) => Buffer.from(arr);
            expect(chainFromSchema.responses.map(mapToBuffer)).toStrictEqual(chain.responses.map(mapToBuffer));
        });
    });
    describe('serialise', () => {
    ...
    describe('verify', () => {
        ...
        test('Malformed responses should be refused', async () => {
            ...
            await expect(async () => chain.verify(ORG_KEY_SPEC, SERVICE_OID, datePeriod)).rejects.toThrowWithMessage(VeraidError, 'At least one of the response messages is malformed');
        });
        test('Chain with missing VeraId TXT should be refused', async () => {
            const { responses } = MOCK_CHAIN.generateFixture(VERAID_RRSET, SecurityStatus.SECURE, datePeriod);
            const incompleteResponses = responses.filter((response) => !response.answersQuestion(VERAID_RECORD.makeQuestion()));
            const responsesSerialised = incompleteResponses.map(serialiseMessage).map(arrayBufferFrom);
            const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
            await expect(async () => chain.verify(ORG_KEY_SPEC, SERVICE_OID, datePeriod)).rejects.toThrowWithMessage(VeraidError, 'Chain is missing VeraId TXT response');
        });
        test('Chain with multiple VeraId TXT responses should be refused', async () => {
            const { responses } = MOCK_CHAIN.generateFixture(VERAID_RRSET, SecurityStatus.SECURE, datePeriod);
            const veraTxtResponse = responses.find((response) => response.answersQuestion(VERAID_RECORD.makeQuestion()));
            const responsesSerialised = [...responses, veraTxtResponse]
                .map(serialiseMessage)
                .map(arrayBufferFrom);
            const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
            await expect(async () => chain.verify(ORG_KEY_SPEC, SERVICE_OID, datePeriod)).rejects.toThrowWithMessage(VeraidError, 'Chain contains multiple VeraId TXT responses');
        });
        describe('Rdata', () => {
            test('Algorithm id should match that of specified key spec', async () => {
                const { responses, trustAnchors } = MOCK_CHAIN.generateFixture(VERAID_RRSET, SecurityStatus.SECURE, datePeriod);
                const responsesSerialised = responses.map(serialiseMessage).map(arrayBufferFrom);
                const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
                const spec = { ...ORG_KEY_SPEC, keyAlgorithm: ORG_KEY_SPEC.keyAlgorithm + 1 };
                await expect(async () => chain.verify(spec, SERVICE_OID, datePeriod, trustAnchors)).rejects.toThrowWithMessage(VeraidError, 'Could not find VeraId record for specified key and/or service');
            });
            ...
            test('Absence of service OID should allow any service', async () => {
                ...
                const { responses, trustAnchors } = MOCK_CHAIN.generateFixture(RrSet.init(record.makeQuestion(), [record]), SecurityStatus.SECURE, datePeriod);
                const responsesSerialised = responses.map(serialiseMessage).map(arrayBufferFrom);
                const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
            ...
            test('Presence of service OID should only allow matching service', async () => {
                const record = VERAID_RECORD.shallowCopy({
                    data: await generateTxtRdata(ORG_KEY_PAIR.publicKey, VERAID_RECORD_TTL_OVERRIDE, SERVICE_OID),
                });
                const { responses, trustAnchors } = MOCK_CHAIN.generateFixture(RrSet.init(record.makeQuestion(), [record]), SecurityStatus.SECURE, datePeriod);
                const responsesSerialised = responses.map(serialiseMessage).map(arrayBufferFrom);
                const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
            ...
            test('Presence of service OID should only deny mismatching service', async () => {
                const record = VERAID_RECORD.shallowCopy({
                    data: await generateTxtRdata(ORG_KEY_PAIR.publicKey, VERAID_RECORD_TTL_OVERRIDE, `1.${SERVICE_OID}`),
                });
                const { responses, trustAnchors } = MOCK_CHAIN.generateFixture(RrSet.init(record.makeQuestion(), [record]), SecurityStatus.SECURE, datePeriod);
                const responsesSerialised = responses.map(serialiseMessage).map(arrayBufferFrom);
                const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
                await expect(async () => chain.verify(ORG_KEY_SPEC, SERVICE_OID, datePeriod, trustAnchors)).rejects.toThrowWithMessage(VeraidError, 'Could not find VeraId record for specified key and/or service');
            });
            test('Explicit service OID should take precedence over wildcard', async () => {
                ...
                const { responses, trustAnchors } = MOCK_CHAIN.generateFixture(RrSet.init(concreteRecord.makeQuestion(), [genericRecord, concreteRecord]), SecurityStatus.SECURE, DatePeriod.init(datePeriod.start, subSeconds(datePeriod.end, concreteRecordTtl + 1)));
                const responsesSerialised = responses.map(serialiseMessage).map(arrayBufferFrom);
                const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
            ...
            test('TTL override should truncate validity period of chain', async () => {
                ...
                const chainPeriod = DatePeriod.init(datePeriod.start, subSeconds(datePeriod.end, ttlOverrideSeconds + 1));
                const { responses, trustAnchors } = MOCK_CHAIN.generateFixture(RrSet.init(record.makeQuestion(), [record]), SecurityStatus.SECURE, chainPeriod);
                const responsesSerialised = responses.map(serialiseMessage).map(arrayBufferFrom);
                const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
                await expect(async () => chain.verify(ORG_KEY_SPEC, SERVICE_OID, datePeriod, trustAnchors)).rejects.toThrowWithMessage(VeraidError, /^VeraId DNSSEC chain is BOGUS: /u);
            });
        });
        describe('DNSSEC', () => {
            test('Invalid chain should be refused', async () => {
                const status = SecurityStatus.INSECURE;
                const { responses, trustAnchors } = MOCK_CHAIN.generateFixture(VERAID_RRSET, status, datePeriod);
                const responsesSerialised = responses.map(serialiseMessage).map(arrayBufferFrom);
                const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
                await expect(async () => chain.verify(ORG_KEY_SPEC, SERVICE_OID, datePeriod, trustAnchors)).rejects.toThrowWithMessage(VeraidError, 
            ...
            test('Lookup errors should be wrapped', async () => {
                ...
                const error = await getPromiseRejection(async () => chain.verify(ORG_KEY_SPEC, SERVICE_OID, datePeriod, trustAnchors), VeraidError);
                expectErrorToEqual(error, new VeraidError('Failed to process DNSSEC verification offline', {
                    cause: expect.any(Error),
                }));
            });
            test('Expired chain should be refused', async () => {
                const pastPeriod = DatePeriod.init(subSeconds(datePeriod.start, 2), subSeconds(datePeriod.start, 1));
                const { responses, trustAnchors } = MOCK_CHAIN.generateFixture(VERAID_RRSET, SecurityStatus.SECURE, pastPeriod);
                const responsesSerialised = responses.map(serialiseMessage).map(arrayBufferFrom);
                const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
                await expect(async () => chain.verify(ORG_KEY_SPEC, SERVICE_OID, datePeriod, trustAnchors)).rejects.toThrowWithMessage(VeraidError, /^VeraId DNSSEC chain is BOGUS: /u);
            });
            test('Chain valid in the future should be refused', async () => {
                const futurePeriod = DatePeriod.init(addSeconds(datePeriod.end, 1), addSeconds(datePeriod.end, 2));
                const { responses, trustAnchors } = MOCK_CHAIN.generateFixture(VERAID_RRSET, SecurityStatus.SECURE, futurePeriod);
                const responsesSerialised = responses.map(serialiseMessage).map(arrayBufferFrom);
                const chain = new VeraidDnssecChain(ORG_DOMAIN, responsesSerialised);
                await expect(async () => chain.verify(ORG_KEY_SPEC, SERVICE_OID, datePeriod, trustAnchors)).rejects.toThrowWithMessage(VeraidError, /^VeraId DNSSEC chain is BOGUS: /u);
            });
...
//# sourceMappingURL=VeraidDnssecChain.spec.js.map