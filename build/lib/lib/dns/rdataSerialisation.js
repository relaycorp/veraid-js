...
function getAlgorithmId(algorithmString) {
    const id = ALGORITHM_ID_BY_STRING[algorithmString];
    if (id === undefined) {
        throw new VeraidError(`Unknown algorithm id ("${algorithmString}")`);
    }
    return id;
}
function validateTtlOverride(ttlOverride) {
    if (ttlOverride < 0) {
        throw new VeraidError(`TTL override must not be negative (got ${ttlOverride})`);
    }
    if (MAX_TTL_OVERRIDE_SECONDS < ttlOverride) {
        throw new VeraidError(`TTL override must not exceed ${MAX_TTL_OVERRIDE_DAYS} days (got ${ttlOverride} seconds)`);
    }
}
function getTtlOverrideFromString(ttlOverrideString) {
...
export async function generateTxtRdata(orgPublicKey, ttlOverride, serviceOid) {
...
export function parseTxtRdata(rdata) {
    ...
    if (fields.length < MIN_RDATA_FIELDS) {
        throw new VeraidError(`RDATA should have at least 3 space-separated fields (got ${fields.length})`);
    }
...
//# sourceMappingURL=rdataSerialisation.js.map