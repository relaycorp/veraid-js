import { dnssecLookUp, Message, Question, RrSet, SecurityStatus, } from '@relaycorp/dnssec';
...
function makeQuestion(domainName) {
    return new Question(`_veraid.${domainName}`, 'TXT');
}
function deserialiseResponses(responsesSerialised) {
...
function getTtlOverrideFromRelevantRdata(responses, veraQuestion, keySpec, serviceOid) {
    ...
    const veraRdataFields = veraRrset.records.map((record) => parseTxtRdata(record.dataFields));
    const relevantRdataSet = veraRdataFields.filter((fields) => fields.keyAlgorithm === keySpec.keyAlgorithm &&
...
function getVerificationPeriod(responses, veraQuestion, keySpec, serviceOid, datePeriod) {
...
export class VeraidDnssecChain {
     ...
     */
    static fromSchema(schema, domainName) {
     ...
     */
    static async retrieve(domainName, { resolver, trustAnchors } = {}) {
        const responses = [];
        const veraQuery = makeQuestion(domainName);
        const serialisingResolver = async (question) => {
    ...
    constructor(domainName, responses) {
     ...
     */
    toSchema() {
        return new DnssecChainSchema(this.responses);
    }
    /**
     ...
     */
    async verify(keySpec, serviceOid, datePeriod, trustAnchors) {
        ...
        const finalPeriod = getVerificationPeriod(responses, veraQuestion, keySpec, serviceOid, datePeriod);
        let dnssecResult;
        try {
...
//# sourceMappingURL=VeraidDnssecChain.js.map