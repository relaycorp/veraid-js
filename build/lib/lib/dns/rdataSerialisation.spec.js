...
import { ORG_KEY_PAIR, ORG_KEY_SPEC, VERAID_RECORD_TTL_OVERRIDE, } from '../../testUtils/veraStubs/organisation.js';
...
describe('generateTxtRdata', () => {
    function splitAndGetField(rdata, index) {
    ...
    describe('TTL override', () => {
        test('Negative values should be refused', async () => {
            ...
            await expect(async () => generateTxtRdata(ORG_KEY_PAIR.publicKey, invalidTtlOverride)).rejects.toThrowWithMessage(VeraidError, `TTL override must not be negative (got ${invalidTtlOverride})`);
        });
        ...
        test('More than 90 days should be refused', async () => {
            ...
            await expect(generateTxtRdata(ORG_KEY_PAIR.publicKey, invalidTtlOverride)).rejects.toThrowWithMessage(VeraidError, `TTL override must not exceed 90 days (got ${invalidTtlOverride} seconds)`);
        });
    });
    describe('Service', () => {
        ...
        test('Service should be specified if set', async () => {
            const rdata = await generateTxtRdata(ORG_KEY_PAIR.publicKey, VERAID_RECORD_TTL_OVERRIDE, SERVICE_OID);
...
describe('parseTxtRdata', () => {
    ...
    test('There should be at least 3 space-separated fields', () => {
        ...
        expect(() => parseTxtRdata(malformedRdata)).toThrowWithMessage(VeraidError, 'RDATA should have at least 3 space-separated fields (got 2)');
    });
    test('Invalid key algorithm should be refused', () => {
        ...
        expect(() => parseTxtRdata(rdata)).toThrowWithMessage(VeraidError, `Unknown algorithm id ("${invalidAlgorithmId}")`);
    });
    describe('TTL override validation', () => {
        ...
        test('Non-integer value should be refused', () => {
            ...
            expect(() => parseTxtRdata(rdata)).toThrowWithMessage(VeraidError, `Malformed TTL override ("${invalidTtlOverride}")`);
        });
    ...
    describe('Input type', () => {
        ...
        test('Empty array should not be supported', () => {
            expect(() => parseTxtRdata([])).toThrowWithMessage(VeraidError, 'TXT rdata array must contain a single item (got 0)');
        });
        test('Array of multiple items should not be supported', () => {
            const rdata = [Buffer.from(rdataString), Buffer.from(rdataString)];
            expect(() => parseTxtRdata(rdata)).toThrowWithMessage(VeraidError, 'TXT rdata array must contain a single item (got 2)');
        });
    });
...
//# sourceMappingURL=rdataSerialisation.spec.js.map