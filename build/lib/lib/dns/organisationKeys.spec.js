...
async function generatePublicKey(algorithm) {
...
describe('getKeySpec', () => {
    test.each([
        ...
        const expectedDigest = calculateDigest(expectedHash, await derSerializePublicKey(publicKey)).toString('base64');
    ...
    test('Unsupported RSA modulus should be refused', async () => {
        ...
        await expect(async () => getKeySpec(publicKey)).rejects.toThrowWithMessage(VeraidError, `RSA key with modulus ${modulusLength} is unsupported`);
    });
    test('Non-RSA keys should be refused', async () => {
        ...
        await expect(async () => getKeySpec(publicKey)).rejects.toThrowWithMessage(VeraidError, `Only RSA-PSS keys are supported (got ${algorithmName})`);
    });
});
//# sourceMappingURL=organisationKeys.spec.js.map