...
import { issueMemberCertificate, selfIssueOrganisationCertificate, MemberIdBundle, SignatureBundle, VeraidDnssecChain, VeraidError, } from '../index.js';
...
const ORG_CERTIFICATE = await selfIssueOrganisationCertificate(TEST_ORG_NAME, TEST_ORG_KEY_PAIR, EXPIRY_DATE);
const MEMBER_CERTIFICATE = await issueMemberCertificate(MEMBER_NAME, MEMBER_KEY_PAIR.publicKey, ORG_CERTIFICATE, TEST_ORG_KEY_PAIR.privateKey, EXPIRY_DATE);
...
describe('main', () => {
    test('Valid signature bundle', async () => {
        const signatureBundle = await SignatureBundle.sign(PLAINTEXT, VERAID_OIDS.TEST_SERVICE, MEMBER_ID_BUNDLE, MEMBER_KEY_PAIR.privateKey, EXPIRY_DATE);
    ...
    test('Invalid signature', async () => {
        ...
        const signatureBundle = await SignatureBundle.sign(PLAINTEXT, VERAID_OIDS.TEST_SERVICE, MEMBER_ID_BUNDLE, otherMemberKeyPair.privateKey, EXPIRY_DATE);
        const signatureBundleSerialised = signatureBundle.serialise();
        const bundle = SignatureBundle.deserialise(signatureBundleSerialised);
        await expect(async () => bundle.verify(PLAINTEXT, VERAID_OIDS.TEST_SERVICE)).rejects.toThrow(VeraidError);
    });
    test('Different service', async () => {
        ...
        await expect(async () => bundle.verify(PLAINTEXT, differentService)).rejects.toThrow(VeraidError);
    });
});
